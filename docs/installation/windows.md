...
Для Windows в качестве основного клиента мы рекомендуем использовать v2RayTun. Это надежный и простой в настройке клиент.

### 1. Скачивание приложения

1. Скачайте приложение по [ссылке](https://files.sberdisk.ru/s/sd3CUKanBvlAWbi) и установите.
...
<div id="auto-setup-container" style="display: none; padding: 20px; background-color: #f0f7fd; border-left: 5px solid #0088cc; margin-bottom: 20px;">
   ...
   <a id="deep-link-btn" href="#" class="btn btn-primary fs-4">Добавить в v2RayTun</a>
   <p class="fs-2 mt-2 text-grey-dk-000">После нажатия подтвердите открытие в приложении v2RayTun.</p>
</div>

<script>
document.addEventListener("DOMContentLoaded", function() {
   const SCHEMA_PREFIX = "v2raytun://import/"; 
...
1. Скопируйте ссылку подписки для Windoiws из нашего [Telegram бота](https://t.me/bez_filtrov_vpn_bot).  
   *(Раздел «Мое подключение» -> «Соединение <ID>»)*.
2. В приложении v2RayTun на главной нажмите «**+**» в правом верхнем углу.

   ![windows1]({{ site.baseurl }}/images/installation/windows/1.png){: width="50%"}
3. В появившемся меню выберите «**Импорт из буфера обмена**»

   ![windows2]({{ site.baseurl }}/images/installation/windows/2.png){: width="50%"}

6. Нажмите переключатель сверху для соединения.

   ![windows3]({{ site.baseurl }}/images/installation/windows/3.png){: width="50%"}